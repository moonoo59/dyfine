-- migration: 0036_update_create_loan_rpc_grace.sql
-- 거치 기간(grace_period_months) 지원을 위해 기존 함수를 DROP 후 재생성

-- 기존 시그니처 1 (0033 버전)
DROP FUNCTION IF EXISTS create_loan(uuid, text, numeric, date, date, int, text, int, numeric, bigint, text, int, uuid);
-- 혹시 모를 더 오래된 버전
DROP FUNCTION IF EXISTS create_loan(uuid, text, numeric, date, date, int, text, int, numeric, bigint, text, int);
...
CREATE OR REPLACE FUNCTION create_loan(
    ...
    p_grace_period_months int DEFAULT 0
)
RETURNS bigint
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    INSERT INTO loans (
        ...
        bank_name, repayment_priority, created_by, grace_period_months
    ) VALUES (
        ...
        p_bank_name, p_repayment_priority, p_created_by, p_grace_period_months
