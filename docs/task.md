...
### Phase 5: 문서 정비 & 마무리 (v3.1)

| # | 대상 | 설명 | 상태 |
|---|---|---|---|
| F-5-1 | `docs/` | 분산된 설계/기본 보고서들을 `01_ARCH_DESIGN`, `02_REQUIREMENTS_API`로 통합 및 불필요 파일 정리. | ✅ |
| F-5-2 | `docs/RELEASE_NOTES.md` | 버전별 통합 릴리즈 노트 생성 및 최신 이력(v3.1) 기록. | ✅ |
| F-5-3 | `README.md` | 프로젝트 소개 및 실행 방법 최신화. | ✅ |
| F-5-3 | [UX/UI] | 매뉴얼 모바일 가로 스크롤 메뉴 및 반응형 버그 수정. | ✅ |
